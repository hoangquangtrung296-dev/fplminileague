            ...
            for (let gw = startGW; gw <= gameweek; gw++) {
                ...
                if (isCompletedGW) {
                    ...
                    if (gwData && gwData.length > 0) {
                        // Check if any entries are missing from cache (players who joined after cache was built)
                        const cachedEntryIds = new Set(gwData.map(e => e.entry));
                        const hasMissingEntries = Object.keys(allEntriesHistory).some(entryId => {
                            const numericId = parseInt(entryId);
                            if (cachedEntryIds.has(numericId)) return false;
                            // Only consider missing if they have history for this GW
                            const entryData = allEntriesHistory[entryId];
                            return entryData && entryData.history && entryData.history.find(h => h.event === gw);
                        });
                        
                        if (hasMissingEntries) {
                            // Invalidate cache so it gets rebuilt from scratch with correct ranks
                            console.log(`⚠️ GW${gw}: cache thiếu player mới, rebuild lại từ đầu`);
                            gwData = null;
                        } else {
